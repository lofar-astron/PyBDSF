...
C     ***  AND W(EMIN) RESPECTIVELY.
      EMAX = DGGDMX + 1
      EMIN = EMAX + 1
C     ***  FOR _USE_ IN RECOMPUTING STEP, THE FINAL VALUES OF LK, UK, DST,
...
C        ***  INDEF. H -- UNDERESTIMATE SMALLEST EIGENVALUE, _USE_ THIS
...
C     ***  _USE_ MORE*S SCHEME FOR INITIALIZING IT.
...
C  ***  SMALLEST EIGENVALUE FOR _USE_ IN UPDATING LK  ***
C
      J = (IRC*(IRC+1))/2
      T = L(J)
      L(J) = ONE
...
C  ***  SINGULAR.  _USE_ ONE STEP OF INVERSE POWER METHOD WITH START
...
C
C  ***  SAVE VALUES FOR _USE_ IN A POSSIBLE RESTART  ***
C
 410  V(DSTNRM) = DST
      V(STPPAR) = ALPHAK
      W(LK0) = LK
