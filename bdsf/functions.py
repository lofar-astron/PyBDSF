...
def set_up_output_paths(opts):
    ...
    # Determine the base directory to use for all output and the log
    # filename. By default, the log file is place in the same directory
    # as the input image unless 'outdir' is explicitly defined. We create
    # the output directory if needed for the log file; otherwise, the
    # output directory is created in output.py only if needed there
    if opts['outdir'] is None:
        basedir = './' + parentname + '_pybdsm/'
        ...
        if not os.path.exists(basedir):
            # Make the output directory, needed for the log file
            os.makedirs(basedir)
    return parentname, basedir, logfilename
